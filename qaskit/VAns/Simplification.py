    ...
    while Count > 0:
        ...
        if new_ansatz[i][0] == 'rx':
            if Control_Bit[1] != -1 and new_ansatz[Control_Bit[1]][0] == 'rx':
                new_parameters[Search_parameters(new_ansatz, Control_Bit[1])] += new_parameters[
                    Search_parameters(new_ansatz, i)]
                new_parameters.pop(Search_parameters(new_ansatz, i))
                new_ansatz.pop(i)
                i = i - 1
    ...
    for i in range(0, len(new_ansatz), 1):
        if new_ansatz[i][0] == 'rx':
            ...
            if Control_Bit[0] != -1 and Quantum_Gate_Marking[0] == 3:
                new_parameters.insert(Search_parameters(new_ansatz, Control_Bit[0]),
                                      new_parameters[Search_parameters(new_ansatz, i)])
        ...
        while k < len(new_ansatz):
            if new_ansatz[k][0] == 'rx':
                ...
                if Control_Bit[1] != -1 and Quantum_Gate_Marking[1] == 3:
                    new_parameters.insert(Search_parameters(new_ansatz, Control_Bit[1]),
                                          new_parameters[Search_parameters(new_ansatz, k)])
